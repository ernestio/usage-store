...
ADD . /go/src/github.com/${GITHUB_ORG:-ernestio}/usage-store
WORKDIR /go/src/github.com/${GITHUB_ORG:-ernestio}/usage-store
...
COPY --from=compiler /go/bin/usage-store .
ENTRYPOINT ["./usage-store"]
